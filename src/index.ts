...
function createMcpServer(): McpServer {
  ...
  // Register the route_task tool
  server.registerTool(
    "route_task",
    {
      title: "Route Task",
      description: "Routes user requests to appropriate internal agents for trend analysis, marketing execution, web design, or graphics design recommendations",
      inputSchema: {
        request: z.string().describe("User request describing the task or opportunity to analyze"),
      },
      outputSchema: {
    ...
    async ({ request }) => {
      ...
      const result = await orchestrator.routeTask(request);
      
      // Add confidence warning if below 70%
      let output = result;
      if (result.confidence < 70) {
        output = {
          ...result,
          recommended_actions: [
            `CONFIDENCE WARNING: Analysis confidence is ${result.confidence}%. Consider gathering more specific information.`,
